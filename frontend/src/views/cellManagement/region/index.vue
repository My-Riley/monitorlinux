      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['camera:region:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="区域地址" prop="address">
              <el-input v-model="form.address" placeholder="请输入区域地址" maxlength="100" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
  ...
  methods: {
    ...
    getList() {
      ...
      listRegion(this.queryParams).then((response) => {
        // 后端返回的已经是树结构，或者需要转换
        // 如果后端返回的是 list，需要 handleTree
        // 之前分析 region_service.py 返回的是 tree 结构，且 key 是 id, label, children
        // 所以直接赋值
    ...
    handleDelete(row) {
      this.$modal
        .confirm('是否确认删除名称为"' + row.label + '"的数据项？')
        .then(function () {
