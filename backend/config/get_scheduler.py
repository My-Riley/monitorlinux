...
class SchedulerUtil:
    ...
    def execute_scheduler_job_once(cls, job_info: JobModel):
        ...
        # 使用一次性任务ID，避免与持久化任务ID冲突
        once_job_id = f"{job_info.job_id}_once"
    ...
    def scheduler_event_listener(cls, event):
        ...
        if hasattr(event, "job_id"):
            job_id = event.job_id
            # 处理一次性任务ID
            real_job_id = job_id.replace("_once", "")
            query_job = cls.get_scheduler_job(job_id=real_job_id)
            if query_job:
                ...
                # 获取调用函数位置参数
                args_list = query_job_info.get("args") or []
                job_args = ",".join([str(arg) for arg in args_list])
                # 获取调用函数关键字参数
                job_kwargs = json.dumps(query_job_info.get("kwargs")) if query_job_info.get("kwargs") else ""
                # 获取任务触发器
                job_trigger = str(query_job_info.get("trigger"))

                # 如果是临时任务，标记一下
                if "_once" in job_id:
                    job_message = f"事件类型: {event_type}, 任务ID: {job_id} (手动执行), 任务名称: {job_name}, 执行于{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                else:
                ...
                # 修正：job_log_service.py里没有 add_job_log_sync 方法，只有 add_job_log_services
                # 但 SchedulerUtil 需要同步调用
                # 我需要在 JobLogDao 中增加同步 add 方法，或者在此处直接调用 DAO 的同步方法
                # JobLogDao 已经在 Step 1046 中增加了 add_job_log_sync
                # 所以这里应该调用 JobLogDao.add_job_log_sync(session, db_job_log)
                # 但要注意 JobLogModel 到 SysJobLog 的转换
                ...
                try:
                    JobLogDao.add_job_log_sync(session, db_job_log)
