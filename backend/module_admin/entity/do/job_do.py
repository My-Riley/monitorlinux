...
class SysJob(Base):
    ...
    job_name = Column(String(64), primary_key=True, nullable=False, default="", comment="任务名称")
    job_group = Column(String(64), primary_key=True, nullable=False, default="default", comment="任务组名")
    ...
    misfire_policy = Column(
        String(20), nullable=True, default="3", comment="计划执行错误策略（1立即执行 2执行一次 3放弃执行）"
    )
    concurrent = Column(CHAR(1), nullable=True, default="1", comment="是否并发执行（0允许 1禁止）")
    status = Column(CHAR(1), nullable=True, default="0", comment="状态（0正常 1暂停）")
...
class SysJobLog(Base):
    ...
    status = Column(CHAR(1), nullable=True, default="0", comment="执行状态（0正常 1失败）")
